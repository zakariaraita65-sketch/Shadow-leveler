...
import { RANK_ORDER, RANK_TITLES, AVAILABLE_TITLES } from "../constants";
...
export default function UserHUD({ stats, rankIndex, userId, questCount = 0, onOpenTitles }: UserHUDProps & { questCount?: number }) {
  ...
  const theme = activeTitleData.theme;
  ...
  return (
    <div className="flex flex-col gap-6 p-6 system-border bg-system-card/50 backdrop-blur-md rounded-xl relative overflow-hidden group shadow-2xl transition-all duration-700" 
         style={{ borderColor: `${theme.color}20`, boxShadow: theme.shadow }}>
      {/* Background Decor */}
      <div className="absolute -top-10 -right-10 w-40 h-40 rounded-full blur-3xl animate-pulse opacity-40" 
           style={{ backgroundColor: theme.color }} />
      <div className="absolute top-0 left-0 w-full h-[1px] shadow-[0_0_10px_rgba(34,211,238,0.5)]" 
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
        <div className="flex flex-col">
          ...
          <h1 className="text-5xl font-display font-black italic tracking-tighter mt-1 leading-none">
            LVL <span style={{ color: theme.color }}>{level}</span>
          </h1>
        </div>
        ...
        <div className="flex items-center gap-6 self-end md:self-center">
          <div className="flex flex-col items-end">
             ...
             >
               <span className="w-1 h-1 rounded-full animate-pulse" style={{ backgroundColor: theme.color }} />
               <span className="text-[10px] font-mono uppercase tracking-[0.3em] font-bold" style={{ color: theme.color }}>
                 {activeTitle}
               </span>
               <Award size={12} className="group-hover/title:scale-110 transition-transform" style={{ color: theme.color }} />
             </button>
      ...
      <div className="flex flex-col gap-2 relative z-10">
        <div className="flex justify-between items-end text-[10px] font-mono uppercase tracking-widest text-white/40">
          <span className="brightness-125 font-bold" style={{ color: theme.color }}>Experience Sync</span>
          <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-2 relative z-10">
        <StatItem icon={<TrendingUp size={14} />} label="STREAK" value={`${streak} DAYS`} color={theme.color} />
        <StatItem icon={<Zap size={14} />} label="FOCUS" value={`${focusTime} MIN`} color={theme.color} />
        <StatItem icon={<Award size={14} />} label="ACTIVE MISSIONS" value={questCount} color={theme.color} />
        <StatItem icon={<Coins size={14} />} label="GOLD" value={`${gold}`} overrideColor="text-yellow-400" />
      </div>
    </div>
  );
}

function StatItem({ icon, label, value, color = "#22d3ee", overrideColor }: { icon: React.ReactNode, label: string, value: any, color?: string, overrideColor?: string }) {
  return (
    <div className="flex items-center gap-3 p-3 rounded-lg border border-white/5 bg-white/5 hover:bg-white/10 transition-all hover:translate-y-[-2px]">
      <div className="opacity-60" style={{ color: overrideColor || color }}>
        {icon}
      </div>
      <div className="flex flex-col">
